...
// items: optional richer format [{ src, title, onClick }]. If omitted, falls back to images array.
const RollingGallery = ({ autoplay = true, pauseOnHover = true, images = IMGS, items }) => {
  const [isScreenSizeSm, setIsScreenSizeSm] = useState(
    typeof window !== 'undefined' ? window.innerWidth <= 640 : false
  );

  const dataItems = items && items.length ? items : images.map((src) => ({ src }));
  const cylinderWidth = isScreenSizeSm ? 1100 : 1800;
  const faceCount = dataItems.length;
  ...
  useEffect(() => {
    ...
    const loop = (ts) => {
      ...
      const delta = -speedRef.current * dt; // negative = rotate left
